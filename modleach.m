...
sink.y=200;
n = 100; %no of nodes
p=0.1; %probability of a node to become cluster head
Eo = 0.5; %energy supplied to each node
Ch = n/10;
ETX=50*0.000000001; %transmitter energy per node
ERX=50*0.000000001; %receiver energy per mode
...
tempi=50;
tempf=200;
%Threshold for transmitting data to the cluster head
...
for i=1:1:n
    ...
    S(i).yd=rand (1, 1)*ym; %it will distribute the nodes in 1 dimension in y axisrandomly
    YR (i)=S (i).yd;
...
countCHs=0; %the number of State flow objects in the current context.
...
for r=0:1:rmax
    ...
    for i = 1:1: n
        S(i).G=0; % it will assign to the nodes that have not been cluster heads.
        %%S (i).cl=0;
    end 
    end
...
for i =1:1:n
    ...
    if (S (i).E<=0) %%%no of dead nodes increased
     dead =dead+1;
        if (dead==1)
...
if S (i).type=='C' && S (i).E>a %%%cluster head count incresed
for j=1:1: ch
    CountCHs=countCHs+1;
    S(i).type='C';
            S(i).G=round (1/p)-1;
